...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorMarginTop = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_top, dp2px(0));
        mIndicatorMarginRight = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_right, dp2px(0));
        mIndicatorMarginBottom = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_bottom, dp2px(0));
...
//            throw new IllegalStateException("TabEntitys can not be NULL or EMPTY !");
        ...
        //只有一个model时不显示tab layout
//        if (tabEntitys.size() == 1) {
//            setVisibility(GONE);
//        } else {
...
//        }
     ...
     * 创建并添加tab
     */
    private void addTab(final int position, final View tabView, int mTabCount) {
        CustomTabEntity tabEntity = mTabEntitys.get(position);
        if (!tabEntity.isVisible()) {
        ...
        if (mTabWidth > 0) {
            ...
            //未指定tab的宽时 需指定tv_tab_title的宽，否则不会自动换行
            //LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(dp2px(100),LinearLayout.LayoutParams.WRAP_CONTENT);
            ViewGroup.LayoutParams params = tv_tab_title.getLayoutParams();
            DisplayMetrics displayMetrics = DensityUtil.getDisplayMetrics(getContext());
            params.width = displayMetrics.widthPixels / mTabCount;
            tv_tab_title.setLayoutParams(params);
        }
...
//            if (!isSelect) {
//                tab_title.setTextColor(mTextUnselectColor);
//            }
    ...
    private void updateTabSelection(int position, boolean isRightVisible) {
        for (int i = 0; i < mTabCount; ++i) {
            View tabView = mTabsContainer.getChildAt(i);
            final boolean isSelect = i == position;
            TextView tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            ...
            ImageView iv_tab_icon = (ImageView) tabView.findViewById(R.id.iv_tab_icon);
            CustomTabEntity tabEntity = mTabEntitys.get(i);
            tab_title.setText(tabEntity.getTabTitle());
            iv_tab_icon.setImageResource(isSelect ? tabEntity.getTabSelectedIcon() : tabEntity.getTabUnselectedIcon());
            if (mTextBold == TEXT_BOLD_WHEN_SELECT) {
                tab_title.getPaint().setFakeBoldText(isSelect);
            }
...
//            });
    ...
    public void setCurrentTab(int currentTab, boolean isRightVisible) {
        mLastTab = this.mCurrentTab;
        this.mCurrentTab = currentTab;
        updateTabSelection(currentTab, isRightVisible);
        if (mIndicatorAnimEnable) {
            calcOffset(currentTab);
        } else {
            invalidate();
